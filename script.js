...
//This is a function named newGame, which runs when you want to start a new Sudoku puzzle.
function newGame(difficulty) {
  ...
  // This function figures out what **numbers can go** in each empty cell, according to Sudoku rules
...
// function to replace char in string as it cant be done directly in js
...
function makeItPuzzle(grid, difficulty) {
  ...
  //maintain the visual symmetry of the Sudoku puzzle
  ...
  //The makeItPuzzle function removes values from the grid randomly, ensuring that the number of empty cells matches the difficulty level.
...
//The ViewPuzzle function then updates the HTML page to display the puzzle grid
...
// READ INPUT Reads the current Sudoku grid values from HTML input fields and returns them as a 2D array (9x9).
function readInput() {
  var result = [];
  for (var i = 0; i < 9; i++) {
    result.push("");
...
// Checks if the entered value is valid, if it conflicts with any other value in the row, column, or block, and if it matches the correct value for the cell.
...
//  Adds or removes CSS classes to change the appearance of a cell based on its validity 
...
// update value of remaining numbers in sudoku
...
function solveSudoku(changeUI) {
  ...
  // read current state and returns a 2d array 
  puzzle = readInput();

  var columns = getColumns(puzzle); //return columns current state
  var blocks = getBlocks(puzzle);  //return blocks current state

  // check if there is any conflict
  var errors = 0;
  var correct = 0;

  // after cheacking below changui to css using 0,1,2,3 states mentioned above
  ...
  // If No Errors, Mark Puzzle as Solvable
  canSolved = true;
  isSolved = true;
  ...
  //shows how many seconds and milliseconds it took.
  if (changeUI)
    document.getElementById("timer").innerText =
      Math.floor(time / 1000) + "." + ("000" + (time % 1000)).slice(-3);


  //It sets flags and returns 3 (which means unsolvable).    
...
function hideMoreOptionMenu() {
  ...
  if (moreOptionList.style.visibility == "visible") {
    moreOptionList.style.maxWidth = "40px"; //fade out animation.
    moreOptionList.style.minWidth = "40px";
    moreOptionList.style.maxHeight = "10px";
    moreOptionList.style.opacity = "0";        //till here
    setTimeout(function () {
      moreOptionList.style.visibility = "hidden"; //completely hiddher here
